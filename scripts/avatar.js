...
export async function Avatar(name, loader) {
  ...
  //   if (child.type == "Bone") {
  //     let names =
  //       child.name.slice(0, -1).replace(/\d/g, "") + child.name.slice(-1);
  //     child.name = names;
  //     // console.log(child.name);
  //     if (child.name.indexOf("mixamorig")) {
...
export function setPose(poseLandmarks, poseWorldLandmarks) {
  ...
  if (rightShoulderVis > VISTHRESH && leftShoulderVis > VISTHRESH) {
    ...
    //                      y - hip -> shoulder,
    //                      z - user -> camera
    let shoulderX = userJoints[RIGHTSHOULDER].clone()
      .sub(userJoints[LEFTSHOULDER])
      .normalize();
    let shoulderY = userJoints[RIGHTSHOULDER].clone()
    ...
    rot = rotateBone(
      ...
      leftWristBone.position,
      basis
    );
    leftElbowBone.quaternion.slerp(rot, SMOOTHING);
    updateBasis(leftElbowBone.quaternion, xAxis, yAxis, zAxis, basis);

    let leftFingersUser = userJoints[LEFTPINKY].lerp(
      userJoints[LEFTINDEX],
      0.5
    );
    let leftFingersAvatar = leftHandBones[PINKY1].position
      .clone()
      .lerp(leftHandBones[INDEX1].position, 0.5);
    rot = rotateBone(
      userJoints[LEFTWRIST],
      leftFingersUser,
      leftFingersAvatar,
    ...
    rot = rotateBone(
      ...
      rightWristBone.position,
      basis
    );
    rightElbowBone.quaternion.slerp(rot, SMOOTHING);
    updateBasis(rightElbowBone.quaternion, xAxis, yAxis, zAxis, basis);

    let rightFingersUser = userJoints[RIGHTPINKY].lerp(
      userJoints[RIGHTINDEX],
      0.5
    );
    let rightFingersAvatar = rightHandBones[PINKY1].position
      .clone()
      .lerp(rightHandBones[INDEX1].position, 0.5);
    rot = rotateBone(
      userJoints[RIGHTWRIST],
      rightFingersUser,
      rightFingersAvatar,
  ...
  if (rightHipVis > VISTHRESH && leftHipVis > VISTHRESH) {
    ...
    //                      y - hip -> shoulder,
    //                      z - user -> camera
    let hipX = userJoints[RIGHTHIP].clone()
      .sub(userJoints[LEFTHIP])
      .normalize();
    let hipY = userJoints[RIGHTSHOULDER].clone()
      .lerp(userJoints[LEFTSHOULDER], 0.5)
    ...
    skeleton.position.x =
      (1 - SMOOTHING) * skeleton.position.x + SMOOTHING * percentX * -1000;
...
export function setMorphs(faceLandmarks) {
  ...
  let pL = new THREE.Vector3(
    faceLandmarks[LEFT].x * WIDTH,
    faceLandmarks[LEFT].y * HEIGHT,
    faceLandmarks[LEFT].z * WIDTH
  );
  let pR = new THREE.Vector3(
    faceLandmarks[RIGHT].x * WIDTH,
    faceLandmarks[RIGHT].y * HEIGHT,
    faceLandmarks[RIGHT].z * WIDTH
  );
...
function smoothRotation(object, rotX, rotY, rotZ) {
  ...
  let SMOOTHING = 0.25;
  if (rotX != 0)
    object.rotation.x = (1 - SMOOTHING) * object.rotation.x + SMOOTHING * rotX;
  if (rotY != 0)
    object.rotation.y = (1 - SMOOTHING) * object.rotation.y + SMOOTHING * rotY;
  if (rotZ != 0)
    object.rotation.z = (1 - SMOOTHING) * object.rotation.z + SMOOTHING * rotZ;
}
...
function setMorphTarget(target, val) {
  ...
  let SMOOTHING = 0.25;
  if (target === "Blink_Left" || target === "Blink_Right") {
    SMOOTHING = 0.4;
  }

  morphTargets[morphDict[target]] =
    (1 - SMOOTHING) * morphTargets[morphDict[target]] + SMOOTHING * val;
}
